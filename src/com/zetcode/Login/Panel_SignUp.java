...
public class Panel_SignUp extends JPanel {
	...
	private void initUI() {
		...
		/**
		 *  회원가입 폼 유효성 검증
		 *  1. 아이디 : DB에 없어야 함. 공백이 아닌 한 자 이상
		 *  2. 패스워드 : 네 자 이상
		 *  4. 닉네임 : DB에 없어야 함.
		 *  
		 *  위 번호의 합을 반환.
		 *  합이
		 *  1:아이디 문제, 2:패스워드 문제, 4:닉네임문제
		 *  3:아이디랑패스워드문제, 6:패스워드랑닉네임문제
		 *  7:다문제
		 *  0:문제 없음.
		 */
	...
	public void setNicknameAvailabilityLabel(boolean onDB) {
		if(onDB) {
			available_Nickname.setForeground(new Color(255, 0, 0));
	...
	public void setIDAvailabilityLabel(boolean onDB) {
		if(onDB) {
			available_ID.setForeground(new Color(255, 0, 0));
